...
def process_pdf(file_path):
    ...
    # Hent ut fakturanummeret
    fakturanummer_match = re.search(r'Fakturanummer:\s*(\d+)', content)
    fakturanummer = fakturanummer_match.group(1) if fakturanummer_match else None
    ...
    fee = re.search(r'Monthly fee\s+\d+\s+(-?\d{1,3}(?:[\s\xa0]\d{3})*(?:,\d{2})?)\s*NOK', content)
    ...
    if vi_betaler:
        data.append({
            ...
            'Fakturanummer': fakturanummer  # Ny kolonne for fakturanummer
        })
    if totalsalg:
        data.append({
            'Selger': selger.group(1) if selger else None,
    ...
    if fee:
        fee_amount = extract_amount(fee.group(0))
        if fee_amount is not None:
            data.append({
                'Selger': selger.group(1) if selger else None,
                'Dato': dato.group(1) if dato else None,
                'Konto': 6551,
                'Beløp': abs(fee_amount),
                'Fakturanummer': fakturanummer
            })
    ...
    if abs(difference) >= Decimal('0.01'):
        if abs(difference) > 10:
            ...
            restbelop = float(difference) - 187.5 if difference > 0 else float(difference) + 187.5
            if restbelop != 0:
                data.append({
                    'Selger': selger.group(1) if selger else None,
                    'Dato': dato.group(1) if dato else None,
